...
export default function InboxPage() {
  ...
  const loadInbox = async () => {
    ...
    if (responsesResult.error) {
      ...
      setError('Postilaatikkoa ei voitu ladata.');
      setLoading(false);
      return;
    }
    ...
    const visibleResponses = responseData.filter((item) => {
      if (item.sender_id === userId && item.sender_deleted_at) return false;
      if (item.owner_id === userId && item.owner_deleted_at) return false;
      return true;
    });
    ...
    if (responseIds.length > 0) {
      ...
      if (messagesResult.error) {
        console.error('Inbox messages fetch error:', messagesResult.error);
      } else {
    ...
    mapped.sort((a, b) => new Date(b.latestAt).getTime() - new Date(a.latestAt).getTime());
      ...
      <div className="space-y-8">
        <section className="rounded-[2rem] bg-white/90 p-6 shadow-lg shadow-slate-200/60 ring-1 ring-slate-200 sm:p-8">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-600">
            Kaikki ilmoituksiin liittyvät keskustelut yhdessä paikassa. Uusimmat ja lukemattomat nousevat listan alkuun.
          </p>
        </section>
        ...
        <section className="space-y-4">
          {conversations.length > 0 ? (
            conversations.map((item) => {
              ...
              const latestFromMe = item.latestSenderId === item.sender_id || item.latestSenderId === item.owner_id
                ? item.latestSenderId !== item.sender_id && item.latestSenderId !== item.owner_id
                  ? false
                  : false
                : false;
              ...
              return (
                <Link
                  ...
                  className={`block rounded-[2rem] border p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${
                    item.unread
                      ? 'border-orange-200 bg-orange-50/90 ring-1 ring-orange-100'
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex shrink-0 flex-col items-end gap-3">
                      {item.unread ? (
                        ...
                        <span className={`rounded-full px-3 py-1 text-xs font-semibold ${getStatusClass(item.workflow_status)}`}>
